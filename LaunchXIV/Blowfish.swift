...
// Forked from the CryptoSwift project.
// Forked because FFXIV requires all calculations to be derived from little endian numbers
// and CryptoSwift baffingly does everything in big endian??
...
extension UInt32 {
    ...
    init<T: Collection>(bytes: T, fromIndex index: T.Index) where T.Element == UInt8, T.Index == Int {
        ...
        let val0 = count > 0 ? UInt32(bigEndian: UInt32(bytes[index.advanced(by: 0)]) << 24) : 0
        let val1 = count > 1 ? UInt32(bigEndian: UInt32(bytes[index.advanced(by: 1)]) << 16) : 0
        let val2 = count > 2 ? UInt32(bigEndian: UInt32(bytes[index.advanced(by: 2)]) << 8) : 0
        let val3 = count > 3 ? UInt32(bigEndian: UInt32(bytes[index.advanced(by: 3)])) : 0
...
public final class Blowfish {
    ...
    fileprivate func encrypt(block: ArraySlice<UInt8>) -> Array<UInt8>? {
        ...
        let lSlice = block[block.startIndex..<block.startIndex.advanced(by: 4)]
        let rSlice = block[block.startIndex.advanced(by: 4)..<block.startIndex.advanced(by: 8)]
        var l = UInt32(bytes: lSlice)
        var r = UInt32(bytes: rSlice)
    ...
    public func encrypt<C: Collection>(_ bytes: C) throws -> Array<UInt8> where C.Element == UInt8, C.Index == Int {
        let bytes = ZeroPadding.add(to: Array(bytes), blockSize: Blowfish.blockSize) // FIXME: Array(bytes) copies

        var out = Array<UInt8>()
        out.reserveCapacity(bytes.count)

        for chunk in bytes.batched(by: Blowfish.blockSize) {
            guard let cipherText = self.encrypt(block: chunk) else {
                out += chunk
                continue
            }
            out += cipherText
    ...
    public func decrypt<C: Collection>(_ bytes: C) throws -> Array<UInt8> where C.Element == UInt8, C.Index == Int {
        ...
        for chunk in Array(bytes).batched(by: Blowfish.blockSize) {
            guard let clearText = self.decrypt(block: chunk) else {
                out += chunk
                continue
            }
            out += clearText
        ...
        out = ZeroPadding.remove(from: out, blockSize: Blowfish.blockSize)

        return out
    }
}

fileprivate class ZeroPadding {
    class func add(to bytes: Array<UInt8>, blockSize: Int) -> Array<UInt8> {
    ...
    class func remove(from bytes: Array<UInt8>, blockSize _: Int?) -> Array<UInt8> {
...
fileprivate struct BatchedCollectionIndex<Base: Collection> {
...
fileprivate protocol BatchedCollectionType: Collection {
    associatedtype Base: Collection
}

fileprivate struct BatchedCollection<Base: Collection>: Collection {
    let base: Base
    let size: Int
